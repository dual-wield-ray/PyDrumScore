...
measures += Measure(ho=[2, 2.5], sd=[1], c1=[1], bd=[2.5, 3.5, 4.5], fm=[3, 4])
...
for _ in range(2):
    for i in [1, 2, 3, 4]:
        m = Measure(ho=note_range(1, end(), 0.5), bd=[1, 3, 4.5], sd=[2, 4])
        if i == 4:
            m.sd = [2, 3, 4]
            m.bd = [1, 2.5, 3.5, 4.5]
            m.c1 = [3, 4]
...
wheres_your_crown = Measure(c1=[1], ho=[2, 3, 4], bd=[1, 4.5], ac=[2, 3, 4])
...
measures += Measure(bd=[1], c1=[1], ho=[2, 3, 4])
...
for i in range(8):
    ...
    if i < 4:
        ...
        m = Measure(sd=note_range(1, end(), 0.25))
    measures += m
